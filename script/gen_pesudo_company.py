...
def generate(i):
    with open("./company.template", "r") as f:
        ...
        with open("pesudo_company/company.target."+`i`, "w") as fw:
...
if __name__ == '__main__':
    ...
    while (i < 100):       
        cmd = "curl -X POST -H 'Content-type:application/json' 127.0.0.1:8001/api/companies/ -d@" + "pesudo_company/company.target."+`i`
        print cmd
        os.system(cmd)
        i = i+1
